...
public function delete($articleid,$userid)
{
    ...
    $sm->bindParam(':articleid', $articleid);
    if (!$sm->execute()) {
        throw new Exception("文章删除失败", IMp_Error::ARTICLE_DELETE_FAIL);
    }
...
public function alist($startIndex,$dataNum)
{

    $sql = "select * from `article` limit :startIndex, :dataNum";

    $sm = $this->_db->prepare($sql);

    $sm->bindParam(':startIndex',$startIndex,PDO::PARAM_INT);
    $sm->bindParam(':dataNum',$dataNum,PDO::PARAM_INT);

    // $sm ->debugDumpParams();

    if (!$sm->execute()) {
         throw new Exception("获取文章失败！", IMp_Error::GET_ARTICLE_FAIL);
    } 
    ...
    if (empty($article)) {
         throw new Exception("文章不存在", IMp_Error::ARTICLEID_NOTEXISTS);
    }
